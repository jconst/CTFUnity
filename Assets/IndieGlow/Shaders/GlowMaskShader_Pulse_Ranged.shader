...
Shader "IndieGlow/ObjectShader/GlowMaskShader_Pulse_Ranged" {
	Properties {
		_MainTex ("Base (RGB)", 2D) = "white" {}
		_TileX ("X Tiling", Float) = 1
		_TileY ("Y Tiling", Float) = 1
		_PulseRate ("Pulse Rate", Range(0.0, 10)) = 1
		_PulseRange ("Pulse Range", Range(0.0, 1)) = 0.5
	...
	SubShader {
		...
		Pass {
    		...
    		half4 frag( v2f v ) : COLOR
			{
				...
				result = tex2D(_MainTex, (v.uv.xy * tiling));
				result.rgb *= _GlowStrength * sin(_Time.w * _PulseRate)*0.5 + _PulseRange;
